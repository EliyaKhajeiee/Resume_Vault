...
-- Create user_resume_access table for tracking free user access
...
-- Users can only view their own resume access records
CREATE POLICY "Users can view own resume access" ON user_resume_access
  FOR SELECT USING (auth.uid() = user_id);

-- Users can insert their own resume access records
